...
src/
  ...
  lib/                   Framework-agnostic "engine" — no React, no DOM lookups
    ...
    algorithms.js          Dijkstra, AStar, BFS, DFS, BellmanFord — each a generator
...
- **Traversal Tree panel**: mirrors the 3D scene — nodes/branches light up in
  the 2D tree exactly when the algorithm visits/relaxes them in 3D.
- Extending with a new algorithm: add a generator class to `algorithms.js`
  emitting the same step shape, then register it in the `ALGORITHMS` map. No
